/// QUOTES
--- Původní skript byl připravený pro transformaci headlines, kde jsme potřebovaly jen titulky článků o Rusku.
--- Podmínku, která je filtruje, teď zakomentuju a nechám u jednotlivých médií všechny články.
------ (kromě parlamentních listů, tam ji musím nechat, je vázaná na datum).
--- Zkontroluju počty řádků.
--- Přidám podmínku na obsah klíčových slov o Rusku v titulku.
--- Vyhodím duplicity.
--- Přidám ID.
...
CREATE OR REPLACE TEMPORARY TABLE aeronet_copy AS
...
CREATE OR REPLACE TEMPORARY TABLE aeronet_kinda_clean AS
...
CREATE OR REPLACE TEMPORARY TABLE aktualne_kinda_clean AS
SELECT 
         "pageAuthor"
       , date AS "pageDate"
       , "pageOpener"
...
CREATE OR REPLACE TEMPORARY TABLE idnes_copy AS
SELECT 
...
CREATE OR REPLACE TEMPORARY TABLE idnes_kinda_clean AS
...
--- 2 Create "kinda clean" table (// including only articles about Russia)
...
CREATE OR REPLACE TEMPORARY TABLE lidovky_copy AS
SELECT "pageAuthor"
...
CREATE OR REPLACE TEMPORARY TABLE lidovky_kinda_clean AS
SELECT
...
CREATE OR REPLACE TEMPORARY TABLE novinky_kinda_clean AS
SELECT DISTINCT
         "pageAuthor"
       , date AS "pageDate"
       , "pageOpener"
...
--- Tady jsem podmínky musela nechat, asi bez ní nefungovalo datum.
--- Parlamentní listy jsou stejně dost velké, tak to celkem dává smysl.
...
CREATE OR REPLACE TEMPORARY TABLE sputnik_copy AS
SELECT "pageAuthor"
...
// SELECT * FROM sputnik_copy; -- 68 352 rows (SPUTNIK originally 72541 rows)


// 6) Only include articles about Russia:
CREATE OR REPLACE TEMPORARY TABLE sputnik_kinda_clean AS
...
--- Bez podmínky: 68 352 rows

// Connect tables and include only articles with keywords related to Russia in title:
CREATE OR REPLACE TEMPORARY TABLE PROJECT_ALL_RUSSIA AS
SELECT * FROM
(SELECT "pageDate", "pageText", "pageTitle", "source" FROM idnes_kinda_clean
UNION ALL
SELECT "pageDate", "pageText", "pageTitle", "source" FROM sputnik_kinda_clean
UNION ALL
SELECT "pageDate", "pageText", "pageTitle", "source" FROM lidovky_kinda_clean
UNION ALL
SELECT "pageDate", "pageText", "pageTitle", "source" FROM aeronet_kinda_clean
UNION ALL
SELECT "pageDate", "pageText", "pageTitle", "source" FROM aktualne_kinda_clean
UNION ALL
SELECT "pageDate", "pageText", "pageTitle", "source" FROM irozhlas_kinda_clean
UNION ALL
SELECT "pageDate", "pageText", "pageTitle", "source" FROM novinky_kinda_clean
UNION ALL
SELECT "pageDate", "pageText", "pageTitle", "source" FROM parlamentnilisty_kinda_clean)
...
OR "pageTitle" ILIKE '%noční vl%' AND "pageText" ILIKE '%rusk%' // ještě doskloňovat
...
// SELECT * FROM PROJECT_ALL_RUSSIA; --- 88 411 rows
// SELECT DISTINCT * FROM PROJECT_ALL_RUSSIA; --- 58 811 rows
...
FROM PROJECT_ALL_RUSSIA
QUALIFY row_number = 1;

// SELECT * FROM PROJECT_ALL_RUSSIA;
--- 58 570 rows
...
CREATE OR REPLACE TABLE PROJECT_QUOTES AS
SELECT CONCAT("source", "rowNum") AS ID
       ,"pageText"
FROM PROJECT_ALL_RUSSIA;

// SELECT * FROM PROJECT_QUOTES
// ORDER BY ID; --- 58 570 rows